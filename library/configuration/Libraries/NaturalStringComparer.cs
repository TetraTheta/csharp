...
public partial class NaturalStringComparer : IComparer<string> {
  private static readonly Regex Chunker = MyRegex();
  ...
  public int Compare(string? x, string? y) {
    ...
    for (int i = 0; i < chunkCount; i++) {
      ...
      if (isXNumber && isYNumber) {
        BigInteger xNum = BigInteger.Parse(xChunk);
        BigInteger yNum = BigInteger.Parse(yChunk);
        cmp = xNum.CompareTo(yNum);
      } else { cmp = string.Compare(xChunk, yChunk, StringComparison.OrdinalIgnoreCase); }
