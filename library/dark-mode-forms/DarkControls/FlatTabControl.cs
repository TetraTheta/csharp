...
public class FlatTabControl : TabControl {
  ...
  protected override void OnMouseMove(MouseEventArgs e) {
    ...
    if (ShowTabCloseButton) {
      ...
      for (int i = 0; i < TabCount; i++) {
        ...
        if (OverCloseTab) { DrawTab(this.CreateGraphics(), this.TabPages[i], i); } else {
          if (TabCloseColor == Color.Red) { DrawTab(this.CreateGraphics(), this.TabPages[i], i); }
        }
      }
    }
    //base.OnMouseMove(e);
  ...
  internal void DrawControl(Graphics g) {
    try {
      ...
      using (Brush bBackColor = new SolidBrush(BackColor)) { g.FillRectangle(bBackColor, ClientRectangle); }
